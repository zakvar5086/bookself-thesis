...
def exact_match(df1, df2, cols, label1, label2):
    ...
    matches = pd.merge(df1, df2, on=norm_cols, how="inner", suffixes=(f"_{label1}", f"_{label2}"))
...
def match_with_strategy(table1, table2, database, strategy, out_dir):
    ...
    if strat["fuzzy"]:
        ...
        fuzzy = fuzzy_match(df1, df2, strat["fuzzy"], strat.get("fuzzy_on"), thresh, table1, table2)
...
def match_custom(table1, table2, database, exact_cols, fuzzy_col, threshold, out_dir):
    ...
    if exact_cols:
        missing = [c for c in exact_cols if c not in df1.columns or c not in df2.columns]
        if missing:
...
def main():
    ...
    parser.add_argument("--database", default="merged", help="Database to use (default: merged)")
    parser.add_argument("--strategy", choices=list(STRATEGIES.keys()), help="Predefined strategy")
    parser.add_argument("--exact-match", nargs="+", metavar="COL", help="Columns for exact match")
    parser.add_argument("--fuzzy-match", metavar="COL", help="Column for fuzzy match")
    parser.add_argument("--threshold", type=float, default=80, help="Fuzzy threshold 0-100 (default: 80)")
    parser.add_argument("--output-dir", help="Output directory (default: matched_results)")
    ...
    cfg = load_config()
    out_dir = Path(args.output_dir) if args.output_dir else Path(cfg["paths"].get("matched_results", "matched_results"))
    ...
    if args.strategy:
        ...
        match_custom(table1, table2, args.database, args.exact_match, args.fuzzy_match, args.threshold, out_dir)
    else:
