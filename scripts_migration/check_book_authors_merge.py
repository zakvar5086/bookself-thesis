...
def main():
    ...
    if len(missing) == 0:
        print(f"[PASS] All unique authors preserved (Source: {len(all_source_keys)}, New: {len(new_keys)})")
        passed += 1
    else:
    ...
    if src1_ids.issubset(mapped_ids_1):
        ...
        print(f"[FAIL] Missing DB1 AuthorIDs")
        failed += 1
    ...
    if src2_ids.issubset(mapped_ids_2):
        ...
        print(f"[FAIL] Missing DB2 AuthorIDs")
        failed += 1
    ...
    if db1_missing.sum() == 0:
        print(f"[PASS] All enriched_db1 rows have new_book_author_id ({len(enriched_db1)} rows)")
        passed += 1
    else:
    ...
    if db2_missing.sum() == 0:
        print(f"[PASS] All enriched_db2 rows have new_book_author_id ({len(enriched_db2)} rows)")
        passed += 1
    else:
    ...
    if len(enriched_db1) == expected_db1:
        print(f"[PASS] DB1: {len(enriched_db1)} enriched + {skipped_db1} skipped + {orphaned_db1} orphaned = {src_db1_count}")
        passed += 1
    else:
    ...
    if len(enriched_db2) == expected_db2:
        print(f"[PASS] DB2: {len(enriched_db2)} enriched + {skipped_db2} skipped + {orphaned_db2} orphaned = {src_db2_count}")
        passed += 1
    else:
    ...
    if len(new_authors) == len(all_source_keys):
        ...
        print(f"[FAIL] BOOK_AUTHORS count mismatch")
        failed += 1
    ...
    if total_orphaned > 0:
        print(f"[WARN] {total_orphaned} orphaned entries filtered (DB1: {orphaned_db1}, DB2: {orphaned_db2})")
    else:
