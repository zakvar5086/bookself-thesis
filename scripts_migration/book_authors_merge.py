...
def load_db_data(db_path):
    ...
    authors = (
        pd.read_csv(a_path, dtype=str)
        if a_path.exists()
        else pd.DataFrame(columns=["AuthorID", "FirstName", "MiddleName", "LastName"])
    )

    ba_path = db_path / "BookAuthors.csv"
    links = (
        pd.read_csv(ba_path, dtype=str)
        if ba_path.exists()
        else pd.DataFrame(columns=["BookAuthorID", "BookID", "AuthorID"])
    )
...
def main():
    ...
    mapping = all_authors[
        ["source_db", "AuthorID", "FirstName", "MiddleName", "LastName", "key"]
    ].copy()
    ...
    ba2_enriched = ba2.merge(
        map_db2[["old_first_name", "old_last_name", "new_book_author_id"]],
        left_on="AuthorID",
        right_index=True,
        how="left",
    )
    ...
    if orphaned_db1 > 0:
        print(
            f"[WARN] Filtering {orphaned_db1} DB1 entries with AuthorIDs not in Authors table"
        )
        ba1_enriched = ba1_enriched[ba1_enriched["new_book_author_id"].notna()].copy()
    if orphaned_db2 > 0:
        print(
            f"[WARN] Filtering {orphaned_db2} DB2 entries with AuthorIDs not in Authors table"
        )
